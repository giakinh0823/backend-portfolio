...
worker2: python -m spacy validate && python -m spacy download en_core_web_sm && python -m spacy link en_core_web_sm en  && celery -A backend worker -l info -O fair --without-gossip --without-mingle --without-heartbeat --concurrency=4 && celery -A backend beat --pool=solo -l info && python manage.py runworker channel_layer -v2 && wait -n
